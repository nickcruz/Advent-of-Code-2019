...
import me.nickcruz.intcode.IntCodeComputer
...
class Day5 {
    ...
    fun turnOnAirConditioner(): Int {
        val computer = IntCodeComputer(TEST_DIAGNOSTIC_PROGRAM)
        val results = computer.run(listOf(AIR_CONDITIONER))
        return results[INDEX_DIAGNOSTIC_CODE]
    }

    fun turnOnThermalRadiatorController(): Int {
        val computer = IntCodeComputer(TEST_DIAGNOSTIC_PROGRAM)
        val results = computer.run(listOf(THERMAL_RADIATOR_CONTROLLER))
        return results[INDEX_DIAGNOSTIC_CODE]
    }

    companion object {
        private const val AIR_CONDITIONER = 1
        private const val THERMAL_RADIATOR_CONTROLLER = 5
        private const val INDEX_DIAGNOSTIC_CODE = 0

        private val TEST_DIAGNOSTIC_PROGRAM = listOf(
            3,
            225,
            1,
            225,
